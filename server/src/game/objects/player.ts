                    ...
                    if (params.gameSourceType) {
                        ...
                        if (weaponDef?.type === "melee") {
                            ...
                            for (let i = 0; i < count; i++) {
                                const commonItems = this.game.lootBarn.getLootTable("tier_world");
                                const commonItem = commonItems[util.randomInt(0, commonItems.length - 1)];
                                this.game.lootBarn.addLoot(commonItem.name, this.pos, this.layer, 1);
                            }

                            if (util.random(0, 1) < 0.1) {
                                const rareGuns = this.game.lootBarn.getLootTable("tier_airdrop_rare");
                                const rareGun = rareGuns[util.randomInt(0, rareGuns.length - 1)];
                                this.game.lootBarn.addLoot(rareGun.name, this.pos, this.layer, 1);
                            }
                        }
                    }
                }
